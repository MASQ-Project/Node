...
impl fmt::Display for NodeReference {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let port_list_string = self.node_addr.ports ().iter ()
            ...
            .collect::<Vec<String>> ()
            .join (",");
        write! (f, "{}:{}:{}", public_key_string, ip_addr_string, port_list_string).unwrap ();
        Ok (())
    }
}
...
impl SubstratumNodeUtils {
    ...
    pub fn socket_addr (node_addr: &NodeAddr, port_selector: PortSelector, name: &str) -> SocketAddr {
        let port_list = node_addr.ports ();
        if port_list.is_empty () {panic! ("{} has no clandestine ports; can't make SocketAddr", name)}
...
mod tests {
    ...
    fn node_reference_can_display_itself () {
        let subject = NodeReference::new (Key::new (&b"Booga"[..]),
            IpAddr::from_str ("12.34.56.78").unwrap (), vec! (1234, 5678));
