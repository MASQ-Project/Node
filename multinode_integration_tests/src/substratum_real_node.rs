...
impl NodeStartupConfig {
    ...
    fn make_args (&self) -> Vec<String> {
        ...
        args.push (match self.node_type {NodeType::Standard => "standard", NodeType::Bootstrap => "bootstrap"}.to_string ());
...
impl SubstratumRealNode {
    ...
    pub fn start (startup_config: NodeStartupConfig, index: usize, host_node_parent_dir: Option<String>) -> SubstratumRealNode {
    ...
    fn extract_node_reference(name: &String) -> Result<NodeReference, String> {
        ...
        loop {
            ...
            match regex.captures(output.as_str()) {
                Some(captures) => {
                    let node_reference = NodeReference::from_str(captures.get(1).unwrap().as_str()).unwrap();
...
mod tests {
    ...
    #[test]
    fn can_make_args () {
        let one_neighbor = NodeReference::new (Key::new (&[1, 2, 3, 4]), IpAddr::from_str ("4.5.6.7").unwrap (), vec! (1234, 2345));
        let another_neighbor = NodeReference::new (Key::new (&[2, 3, 4, 5]), IpAddr::from_str ("5.6.7.8").unwrap (), vec! (3456, 4567));
        let one_bootstrap_from = NodeReference::new (Key::new (&[3, 4, 5, 6]), IpAddr::from_str ("6.7.8.9").unwrap (), vec! (5678, 6789));
        let another_bootstrap_from = NodeReference::new (Key::new (&[4, 5, 6, 7]), IpAddr::from_str ("7.8.9.10").unwrap (), vec! (7890, 8901));
        ...
        assert_eq! (result, Command::strings (vec! (
            "--ip", "255.255.255.255",
            "--dns_servers", "8.8.8.8",
            "--neighbor", format! ("{}", one_neighbor).as_str (),
            "--neighbor", format! ("{}", another_neighbor).as_str (),
            "--bootstrap_from", format! ("{}", one_bootstrap_from).as_str (),
            "--bootstrap_from", format! ("{}", another_bootstrap_from).as_str (),
            "--node_type", "standard",
            "--port_count", "1",
