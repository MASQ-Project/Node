...
impl MASQNodeUIClient {
    ...
    fn wait_for_message (&self, path: MessagePath, timeout: Duration) -> MessageBody {
        ...
        loop {
            match self.check_for_waiting_message(&mut inner) {
                ...
                None => if let Some (target) = target_opt {
                    return target
                }
                else {
    ...
    fn check_for_buffered_message (&self, path: MessagePath) -> Option<MessageBody> {
        let mut inner = self.inner.borrow_mut();
        let (target_opt, new_buffer) = inner.buffer.drain(..).fold((None, vec![]), |so_far, msg| {
            if msg.path == path {
                (Some(msg), so_far.1)
            }
            else {
    ...
    fn check_for_waiting_message (&self, inner: &mut RefMut<MASQNodeUIClientInner>) -> Option<MessageBody> {
