...
// NOTE: It's very important that this never be cloned; then multiple SubstratumMockNodes will point to the same
...
impl Drop for SubstratumMockNode {
    fn drop(&mut self) {
        self.stop ();
    }
}
...
impl SubstratumMockNode {
    ...
    pub fn transmit_package (&self, transmit_port: u16, package: IncipientCoresPackage, masquerader: &Masquerader, target_key: &Key, target_addr: SocketAddr) -> Result<(), io::Error> {
        let (lcp, _) = LiveCoresPackage::from_incipient (package, self.cryptde.as_ref ());
            ...
            match framer.take_frame () {
                ...
                None => {
                    match control_stream.read (&mut buf) {
                        ...
                        Err (e) => {println! ("No data from {} after {:?}", self.name (), timeout); return Err (e)},
    ...
    pub fn wait_for_package (&self, masquerader: &Masquerader, timeout: Duration) -> Result <(SocketAddr, SocketAddr, ExpiredCoresPackage), io::Error> {
        ...
        Ok ((data_hunk.from, data_hunk.to, lcp.to_expired (self.cryptde.as_ref ())))
    }
    ...
    fn wait_for_startup(wait_addr: SocketAddr, name: &String) -> TcpStream {
        ...
        loop {
            match TcpStream::connect(wait_addr) {
                Ok(s) => {println! ("{} startup detected on {}", name, wait_addr); stream = Some (s); break},
                Err(e) => {println! ("{} not yet started on {}: {}", name, wait_addr, e); ()},
            }
            retries -= 1;
            if retries <= 0 { break }
