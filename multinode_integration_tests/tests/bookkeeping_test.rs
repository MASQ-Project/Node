...
fn provided_services_are_recorded_in_accounts_receivable() {
    ...
    let param_block = ParamBlock {
        ...
        consuming_wallet: originating_node.consuming_wallet().as_ref().unwrap ().clone (),
    };
    let originating_node = cluster
        .get_real_node_by_key(&bootstrap.originating_node_key())
        .unwrap();
    ...
    check_routing_charges (routing_node, &param_block, &originating_node.cryptde (), &exit_node.cryptde());
...
fn check_routing_charges (routing_node: SubstratumRealNode, param_block: &ParamBlock, originating_cryptde: &CryptDE, exit_cryptde: &CryptDE) {
    let account_status = account_status (&routing_node, &param_block.consuming_wallet).unwrap ();

    let (request_bytes, expected_request_charge) = calculate_request_routing_charge(param_block.request_len, exit_cryptde);
    let (response_bytes, expected_response_charge) = calculate_response_routing_charge(param_block.response_len, originating_cryptde);
    assert_eq!(
        account_status.balance as u64,
        expected_request_charge + expected_response_charge,
        "Balance should be calculated for 2 routing services and {} + {} bytes", request_bytes, response_bytes
    );
    timestamp_between (&param_block.before, &account_status.last_received_timestamp, &param_block.after);
}
...
fn check_exit_charges (exit_node: SubstratumRealNode, param_block: &ParamBlock) {
    ...
    assert_eq!(
        account_status.balance as u64,
        expected_request_charge + expected_response_charge,
        "Balance should be calculated for 2 exit services and {} + {} bytes", param_block.request_len, param_block.response_len
    );
    timestamp_between (&param_block.before, &account_status.last_received_timestamp, &param_block.after);
}
...
fn account_status (node: &SubstratumRealNode, consuming_wallet: &Wallet) -> Option<Account> {
    ...
    receivable_dao
        .account_status(consuming_wallet)
}
...
fn calculate_request_routing_charge(bytes: usize, exit_cryptde: &CryptDE) -> (usize, u64) {
    ...
    let payload_enc = exit_cryptde.encode (&exit_cryptde.public_key (), &payload_ser).unwrap ();
    let payload_len = payload_enc.len ();
    (payload_len, TEMPORARY_PER_ROUTING_RATE + (TEMPORARY_PER_ROUTING_BYTE_RATE * payload_len as u64))
}
...
fn calculate_response_routing_charge(bytes: usize, originating_cryptde: &CryptDE) -> (usize, u64) {
    ...
    let payload_ser = PlainData::from (serde_cbor::ser::to_vec (&payload).unwrap ());
    let payload_enc = originating_cryptde.encode (&originating_cryptde.public_key (), &payload_ser).unwrap ();
    let payload_len = payload_enc.len ();
    (payload_len, TEMPORARY_PER_ROUTING_RATE + (TEMPORARY_PER_ROUTING_BYTE_RATE * payload_len as u64))
}
...
fn timestamp_between (before: &SystemTime, timestamp: &SystemTime, after: &SystemTime) {
    ...
    assert!(timestamp_t >= before_t, "{} should have been after {}, but wasn't", timestamp_t, before_t);
    assert!(timestamp_t <= after_t, "{} should have been before {}, but wasn't", timestamp_t, after_t);
}

fn make_request_payload(bytes: usize, cryptde: &CryptDE) -> ClientRequestPayload {
    ClientRequestPayload {
        stream_key: StreamKey::new(cryptde.public_key(), SocketAddr::from_str("1.2.3.4:5678").unwrap()),
