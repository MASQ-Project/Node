...
use multinode_integration_tests_lib::masq_real_node::{
    ConsumingWalletInfo, EarningWalletInfo, NodeStartupConfigBuilder, MASQRealNode,
};
...
fn verify_bill_payment() {
    ...
    let consuming_node_path =
        MASQRealNode::node_home_dir(&project_root, &consuming_node_name);
    ...
    let serving_node_1_path =
        MASQRealNode::node_home_dir(&project_root, &serving_node_1_name);
    ...
    let serving_node_2_path =
        MASQRealNode::node_home_dir(&project_root, &serving_node_2_name);
    ...
    let serving_node_3_path =
        MASQRealNode::node_home_dir(&project_root, &serving_node_3_name);
