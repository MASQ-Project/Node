...
fn neighborhood_notified_of_missing_node_when_connection_refused () {
    ...
    let subject = cluster.start_real_node (NodeStartupConfigBuilder::standard ()
        ...
        .build ());
    let _bootstrap_gossip_package = mock_bootstrap.wait_for_package (&JsonMasquerader::new (), Duration::from_millis (1000)).unwrap ();
    ...
    let cores_package = GossipBuilder::new ()
        ...
        .add_fictional_node (NodeRecordInner {public_key: refusing_node_key.clone (),
    ...
    let cores_package = IncipientCoresPackage::new (
        Route::new (
            vec! (RouteSegment::new (vec! (&mock_bootstrap.public_key (),
                                           &subject.public_key (), &refusing_node_key), Component::ProxyClient)),
    ...
    let _rebroadcast_bootstrap_gossip_package = mock_bootstrap.wait_for_package(
        &JsonMasquerader::new(), Duration::from_millis(1000));

    let (_, _, new_neighborhood_gossip_package) = mock_bootstrap.wait_for_package(&JsonMasquerader::new (), Duration::from_millis (1000)).unwrap();
    ...
    let subject_record = gossip.node_records.iter().find(|&x| x.inner.public_key == subject.public_key()).expect("should have the subject node record");

    assert!(!subject_record.inner.neighbors.contains(&refusing_node.public_key()));
}
