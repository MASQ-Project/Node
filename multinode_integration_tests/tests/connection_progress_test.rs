...
use masq_lib::messages::{FromMessageBody, UiConnectionChangeBroadcast, UiConnectionChangeStage};
...
fn connection_progress_is_properly_broadcast() {
    ...
    let message_body = ui_client.wait_for_specific_broadcast(
        vec!["connectionChange"],
        Duration::from_secs(5)
    );
    let (ccb, _) = UiConnectionChangeBroadcast::fmb(message_body).unwrap();
    if ccb.stage == UiConnectionChangeStage::ConnectedToNeighbor {
        let message_body = ui_client.wait_for_specific_broadcast(
            vec!["connectionChange"],
            Duration::from_secs(5),
        );
        let (ccb, _) = UiConnectionChangeBroadcast::fmb(message_body).unwrap();
        assert_eq! (ccb.stage, UiConnectionChangeStage::ThreeHopsRouteFound);
    }
    else {
        assert_eq! (ccb.stage, UiConnectionChangeStage::ThreeHopsRouteFound);
    }
