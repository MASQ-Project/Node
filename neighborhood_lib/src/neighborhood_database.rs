...
impl NeighborhoodDatabase {
    pub fn new (public_key: &Key, node_addr: &NodeAddr, is_bootstrap_node: bool, cryptde: &CryptDE) -> NeighborhoodDatabase {
        ...
        let complete_signature = NeighborhoodDatabase::sign(&inner, cryptde);
        let obscured_signature = NeighborhoodDatabase::sign(&obscured_inner, cryptde);

        result.add_node (&NodeRecord::new (public_key, Some(node_addr), is_bootstrap_node, Some(complete_signature), Some(obscured_signature))).expect ("Unable to add self NodeRecord to Neighborhood");
        result
    }

    fn sign(inner: &NodeRecordInner, cryptde: &CryptDE) -> CryptData {
        let serialized = match serde_cbor::ser::to_vec(inner) {
            Ok(inner) => inner,
            Err(_) => {
                panic!("SubstratumNode initialization failed: could not serialize NodeRecord")
            }
        };
        ...
        cryptde.sign(&PlainData::new(&hash.digest().bytes())).expect("SubstratumNode initialization failed: could not sign NodeRecord")
    }
