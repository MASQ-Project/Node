...
impl ProxyServer {
    ...
    fn report_routing_service(
        ...
        let earning_wallets: Vec<&Wallet> = response
            ...
            .collect();
        if earning_wallets.is_empty() {
            logger.debug("No routing services requested.".to_string());
        }
        earning_wallets.into_iter().for_each(|earning_wallet| {
...
mod tests {
    ...
    fn proxy_server_receives_http_request_with_new_stream_key_from_dispatcher_then_sends_cores_package_to_hopper(
        ...
        thread::spawn(move || {
            ...
            let mut peer_actors = peer_actors_builder().hopper (hopper_mock).neighborhood(neighborhood_mock).build ();
    ...
    #[test]
    fn proxy_server_sends_message_to_accountant_for_routing_service_consumed() {
        let cryptde = cryptde();
        ...
        thread::spawn(move || {
            ...
            subject.stream_key_factory = Box::new(stream_key_factory);
            let subject_addr: Addr<Syn, ProxyServer> = subject.start();
            let mut peer_actors = peer_actors_builder().accountant (accountant_mock).neighborhood(neighborhood_mock).build ();
            peer_actors.proxy_server = ProxyServer::make_subs_from(&subject_addr);
            subject_addr.try_send(BindMessage { peer_actors }).unwrap();
            subject_addr.try_send(msg_from_dispatcher).unwrap();
            system.run();
    ...
    fn proxy_server_receives_http_request_from_dispatcher_but_neighborhood_cant_make_route() {
        ...
        thread::spawn(move || {
            ...
            let subject_addr: Addr<Syn, ProxyServer> = subject.start();
            let mut peer_actors = peer_actors_builder().dispatcher (dispatcher).neighborhood(neighborhood_mock).build ();
            peer_actors.proxy_server = ProxyServer::make_subs_from(&subject_addr);
            subject_addr.try_send(BindMessage { peer_actors }).unwrap();
    ...
    fn proxy_server_receives_tls_client_hello_from_dispatcher_but_neighborhood_cant_make_route() {
        ...
        thread::spawn(move || {
            ...
            let subject_addr: Addr<Syn, ProxyServer> = subject.start();
            let mut peer_actors = peer_actors_builder ().dispatcher (dispatcher).neighborhood (neighborhood).build ();
            peer_actors.proxy_server = ProxyServer::make_subs_from(&subject_addr);
            subject_addr.try_send(BindMessage { peer_actors }).unwrap();
