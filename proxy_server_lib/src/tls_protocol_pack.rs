...
impl ProtocolPack for TlsProtocolPack {
    fn proxy_protocol(&self) -> ProxyProtocol {ProxyProtocol::TLS}

    fn find_host_name(&self, data: &PlainData) -> Option<String> {
        if !TlsProtocolPack::is_handshake (&data) {return None}
        if !TlsProtocolPack::is_client_hello (&data) {return None}
...
impl TlsProtocolPack {
    ...
    fn find_host_name(data: &PlainData) -> Option<String> {
        ...
        let compression_methods_offset = TlsProtocolPack::advance_past (data, cipher_suites_offset, 2)?;
        ...
        while extension_offset < extensions_end {
            ...
            if extension_type == 0x0000 {return TlsProtocolPack::host_name_from_extension (data, extension_offset)}
    ...
    fn host_name_from_extension (data: &PlainData, offset: usize) -> Option<String> {
        ...
        while server_name_list_entry_offset < server_name_list_end {
            ...
            if server_name_type == 0x00 {return TlsProtocolPack::host_name_from_list_entry (data, server_name_list_entry_offset)}
            server_name_list_entry_offset = TlsProtocolPack::advance_past (data, server_name_list_entry_offset + 1, 2)?;
        }
        None
    }
    ...
    fn host_name_from_list_entry (data: &PlainData, offset: usize) -> Option<String> {
        ...
        match String::from_utf8 (Vec::from (&data.data[(server_name_offset)..(server_name_offset + server_name_length)])) {
    ...
    fn advance_past (data: &PlainData, length_offset: usize, length_length: usize) -> Option<usize> {
        if length_offset + length_length > data.data.len () {return None}
        let length = if length_length == 1 {TlsProtocolPack::u8_from (data, length_offset)} else {TlsProtocolPack::u16_from (data, length_offset)};
...
mod tests {
    ...
    fn rejects_packet_that_is_not_client_hello () {
        vec! (0u8, 2u8, 11u8, 12u8, 13u8, 14u8, 15u8, 16u8, 20u8).iter ().for_each (|handshake_type| {
    ...
    fn does_not_panic_for_hostname_that_is_not_utf8 () {
        let data = PlainData::new (&[
            ...
            's' as u8, 'e' as u8, 'r' as u8, 'v' as u8, 'e' as u8, 'r' as u8, '.' as u8, 0xC3, 0x28, 'm' as u8, // bad server_name
        ]);
