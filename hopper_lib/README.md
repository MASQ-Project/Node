...
into the same top-level directory as `hopper_lib`:

- Workspace/
    - substratum_sub_lib/
    - substratum_hopper_lib/
        
You'll also need an Internet connection when you build `hopper_lib`, so that `cargo` can pull down dependencies.

## How To
We build and run tests for `hopper_lib` using bash scripts located in the `ci` directory.
