...
pub const DATA_DIRECTORY_HELP: &str =
    "Directory in which the Node will store its persistent state, including at least its database and \
    ...
    changed to: '/home/username/.local/share/MASQ/polygon-mumbai'.\
    ...
    to correspond last part with the chain name. If you start Node without the Daemon, from the command line, \
    your data directory is taken as it is. So be careful, because if you specify data-directory to the path, \
    where is located database of different Chain, you will be facing panic and the Node will be terminated.";
