...
use crate::protocols::utils::{Direction, Packet, PCP_HEADER, UnrecognizedData};
...
pub fn test_pcp(socket:UdpSocket, router_address: SocketAddr,router_ip:IpAddr){
    ...
    println!("{}",PCP_HEADER);
    socket.set_read_timeout(Some(Duration::new(10,0))).expect("setting socket timeout failed");
    match socket.recv_from (&mut buf) {
        Ok(length) => {
            ...
            if packet.direction != Direction::Response {report
                .push_str("Additional issue: Response packet was a request, not a response")};
            if packet.opcode != Opcode::Announce {report
                .push_str(&format!("Additional issue: Response packet opcode was {:?}, not Announce", packet.opcode))};
            if packet.result_code_opt != Some (0) {report
                .push_str(&format!("Additional issue: Response packet result code was {:?}, not 0", packet.result_code_opt))};
