...
use automap_lib::comm_layer::{Transactor, AutomapError};
...
fn test_pcp (router_ip: IpAddr) {
    ...
    if status.cumulative_success {
        println! ("====== PCP is implemented on your router and we can successfully employ it ======\n")
    }
    else {
...
fn test_igdp (router_ip: IpAddr) {
    ...
    let status = if status.step_success {
        ...
        if status.step_error.as_ref().expect ("Step failure, but no error recorded!") == &AutomapError::AddMappingError("OnlyPermanentLeasesSupported".to_string()) {
...
fn seek_public_ip(status: TestStatus, router_ip: IpAddr, transactor: &dyn Transactor) -> TestStatus {
...
fn poke_firewall_hole(status: TestStatus, router_ip: IpAddr, transactor: &dyn Transactor) -> (u16, TestStatus) {
    if status.fatal {return (0, status)}
    ...
    let _socket = match UdpSocket::bind (SocketAddr::new (IpAddr::V4(Ipv4Addr::new (0, 0, 0, 0)), port)) {
        ...
        Err (e) => {
            ...
            return (port, status.abort(AutomapError::SocketBindingError(format!("{:?}", e), socket_addr)))
        },
    };
    println! ("{}. Poking a 3-second hole in the firewall for port {}...", status.step, port);
    let timer = Timer::new();
    match transactor.add_mapping (router_ip, port, 5) {
        Ok (delay) => {
            println! ("...success after {}! Recommended remap delay is {} seconds.", timer.ms(), delay);
            (port, status.succeed())
        },
        Err (e) => {
            println! ("...failed after {}: {:?}", timer.ms(), e);
...
fn poke_permanent_firewall_hole(status: TestStatus, router_ip: IpAddr, transactor: &dyn Transactor) -> (u16, TestStatus) {
    if status.fatal {return (0, status)}
    let port = find_free_port();
    let socket_addr = SocketAddr::new (IpAddr::V4(Ipv4Addr::new (0, 0, 0, 0)), port);
    let _socket = match UdpSocket::bind (SocketAddr::new (IpAddr::V4(Ipv4Addr::new (0, 0, 0, 0)), port)) {
    ...
    println! ("{}. Poking a permanent hole in the firewall for port {}...", status.step, port);
    let timer = Timer::new();
...
fn remove_firewall_hole(port: u16, status: TestStatus, router_ip: IpAddr, transactor: &dyn Transactor) -> TestStatus {
    if status.fatal {return status}
    println! ("{}. Removing the port-{} hole in the firewall...", status.step, port);
    let timer = Timer::new();
...
fn remove_permanent_firewall_hole(port: u16, status: TestStatus, router_ip: IpAddr, transactor: &dyn Transactor) -> TestStatus {
    if status.fatal {return status}
    println! ("{}. Removing the port-{} hole in the firewall...", status.step, port);
    let timer = Timer::new();
    match transactor.delete_mapping (router_ip, port) {
        ...
        Err (e) => {
            ...
            println! ("close. You'll need to close it yourself in your router's administration pages.");
