...
use node_lib::ui_gateway::messages::{UiSetup, UiShutdownOrder, UiStartOrder, UiStartResponse, UiRedirect, UiFinancialsRequest, NODE_NOT_RUNNING_ERROR};
use std::ops::Add;
...
eprintln! ("Sending first financials request");
    let not_running_financials_response = initialization_client.transact_with_context_id::<UiFinancialsRequest, UiRedirect>(financials_request.clone(), context_id).unwrap_err();
...
eprintln! ("Sending second financials request");
    let running_financials_response: UiRedirect = initialization_client.transact_with_context_id(financials_request.clone(), context_id).unwrap();

eprintln! ("Asserting responses"); // Never gets here
    assert_eq! (not_running_financials_response.0, NODE_NOT_RUNNING_ERROR);
    assert_eq! (not_running_financials_response.1, "Cannot handle financials request: Node is not running".to_string());
    assert_eq! (running_financials_response.opcode, "financials".to_string());
    assert_eq! (running_financials_response.port, start_response.redirect_ui_port);
    ...
    let actual_payload: UiFinancialsRequest = serde_json::from_str (&running_financials_response.payload_json).unwrap();
