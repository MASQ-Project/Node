...
use crate::blockchain::blockchain_interface::{MAINNET_CONTRACT_ADDRESS, MULTINODE_TESTNET_CONTRACT_ADDRESS, ROPSTEN_TESTNET_CONTRACT_ADDRESS, RINKEBY_TESTNET_CONTRACT_ADDRESS};
use crate::sub_lib::neighborhood::Blockchain;
use web3::types::Address;
use crate::blockchain::blockchains_specific_constants::{MAINNET_CONTRACT_CREATION_BLOCK, ROPSTEN_TESTNET_CONTRACT_CREATION_BLOCK, RINKEBY_TESTNET_CONTRACT_CREATION_BLOCK, MULTINODE_TESTNET_CONTRACT_CREATION_BLOCK};
...
where F: FnMut(&&BlockchainRecord)->bool{
    CHAINS.iter().find(closure).expect("chain outside the bounds; unknown")
}
...
fn return_right_record_by_chain_name(chain_name: &str)->&'static BlockchainRecord{
    return_right_record(Box::new(|b:&&BlockchainRecord|b.in_command_name == chain_name))
}

fn return_right_record_by_blockchain_name(blockchain_name: Blockchain)->&'static BlockchainRecord{
    return_right_record(Box::new(|b:&&BlockchainRecord|b.non_num_chain_id == blockchain_name))
}
...
#[cfg(test)]
mod tests {
    use crate::blockchain::blockchain_interface::{MAINNET_CONTRACT_ADDRESS, RINKEBY_TESTNET_CONTRACT_ADDRESS, ROPSTEN_TESTNET_CONTRACT_ADDRESS, MULTINODE_TESTNET_CONTRACT_ADDRESS};
    use crate::blockchain::blockchains::{BlockchainRecord, CHAINS, contract_address, contract_creation_block_from_chain_id, chain_name_from_id, blockchain_from_chain_id, chain_id_from_name, chain_name_from_blockchain, chain_id_from_blockchain, platform_from_chain_name, return_right_record};
    use crate::sub_lib::neighborhood::Blockchain;
    use crate::blockchain::blockchains_specific_constants::{MAINNET_CONTRACT_CREATION_BLOCK, ROPSTEN_TESTNET_CONTRACT_CREATION_BLOCK, RINKEBY_TESTNET_CONTRACT_CREATION_BLOCK, MULTINODE_TESTNET_CONTRACT_CREATION_BLOCK};
    use web3::types::Address;
    ...
    fn assert_contract_from_chain_id(blockchain: Blockchain, expected_contract:Address){
        assert_eq!(contract_address(chain_id_from_blockchain(blockchain)),expected_contract)
    }

    #[test]
    fn contract_creation_block_works(){
        assert_creation_block_from_chain_id(Blockchain::EthMainnet, MAINNET_CONTRACT_CREATION_BLOCK);
        assert_creation_block_from_chain_id(Blockchain::EthRopsten, ROPSTEN_TESTNET_CONTRACT_CREATION_BLOCK);
        assert_creation_block_from_chain_id(Blockchain::EthRinkeby, RINKEBY_TESTNET_CONTRACT_CREATION_BLOCK);
        assert_creation_block_from_chain_id(Blockchain::Dev, MULTINODE_TESTNET_CONTRACT_CREATION_BLOCK);
        assert_eq!(CHAINS.len(),4)
    }

    fn assert_creation_block_from_chain_id(blockchain: Blockchain, expected_block_number:u64){
        assert_eq!(contract_creation_block_from_chain_id(chain_id_from_blockchain(blockchain)),expected_block_number)
    }
    ...
    fn assert_chain_name_from_chain_id(blockchain: Blockchain, expected_name:&str){
        assert_eq!(chain_name_from_id(chain_id_from_blockchain(blockchain)),expected_name)
    }
    ...
    fn assert_blockchain_from_chain_id(blockchain: Blockchain, expected_blockchain:Blockchain){
        assert_eq!(blockchain_from_chain_id(chain_id_from_blockchain(blockchain)),expected_blockchain)
    }
    ...
    fn assert_chain_id_from_name(blockchain: Blockchain, expected_id:u8){
        assert_eq!(chain_id_from_name(chain_name_from_blockchain(blockchain)),expected_id)
    }
    ...
    fn assert_platform_from_name(blockchain: Blockchain, expected_platform:&str){
        assert_eq!(platform_from_chain_name(chain_name_from_blockchain(blockchain)),expected_platform)
    }
}
