...
impl DbInitializerReal {
    ...
    fn initialize_config(&self, conn: &Connection, chain_id: u8) {
        ...
        Self::set_config_value(
            conn,
            "chain_name",
            Some(Chain::from_id(chain_id).record().plain_text_name),
            false,
...
mod tests {
    ...
    fn existing_database_with_the_wrong_version_comes_to_migrator_that_makes_it_gradually_migrate_to_upper_versions(
        ...
        let _ = subject
            .initialize(&updated_db_path_dir, TEST_DEFAULT_CHAIN_ID, true)
            .unwrap();
        let _ = subject
            .initialize(&from_scratch_db_path_dir, TEST_DEFAULT_CHAIN_ID, true)
            .unwrap();
