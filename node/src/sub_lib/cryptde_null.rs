...
mod tests {
    ...
    fn symmetric_encryption_fails_with_different_keys() {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| subject.decode_sym(&key2, &encrypted_data))).unwrap_err();
        ...
        assert_eq!(panic_msg, &format!("Could not decrypt with {} data beginning with {}", wrong_key, data));
    }
