...
mod tests {
    ...
    fn can_construct_hop() {
        ...
        let subject = LiveHop::new(
            ...
            Some(make_paying_wallet(b"wallet"))
                .map(|w| w.as_payer(&key, &Chain::from_id(TEST_DEFAULT_CHAIN_ID).record().contract)),
    ...
    fn encode_decode() {
        ...
        let contract_address = &Chain::from_id(TEST_DEFAULT_CHAIN_ID).record().contract.clone();
