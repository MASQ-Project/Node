...
impl ParseTools {
    ...
    pub fn get_string_field (map: &serde_json::map::Map<String, Value>, name: &str) -> Option<String> {
        let value = map.get (name)?;
        match value {
    ...
    pub fn get_array_field<'a> (map: &'a serde_json::map::Map<String, Value>, name: &str) -> Option<&'a Vec<Value>> {
        let value = map.get (name)?;
        match value {
    ...
    pub fn get_object_field<'a> (map: &'a serde_json::map::Map<String, Value>, name: &str) -> Option<&'a serde_json::map::Map<String, Value>> {
        let value = map.get (name)?;
        match value {
    ...
    pub fn get_object_element<'a> (array: &'a Vec<Value>, index: usize) -> Option<&'a serde_json::map::Map<String, Value>> {
        let value = array.get(index)?;
        match value {
            Value::Object (v) => Some (v),
            _ => None
...
mod tests {
    ...
    fn get_bool_field_handles_good_value () {
        ...
        map.insert ("number".to_string(), Value::Number(Number::from_f64(1.23).unwrap()));
        map.insert ("string".to_string(), Value::String("booga".to_string()));
        map.insert ("array".to_string(), Value::Array(vec![Value::Bool(true), Value::Bool(false)]));
        ...
        assert_eq! (ParseTools::get_string_field (&map, "string"), Some("booga".to_string()));
        assert_eq! (ParseTools::get_array_field (&map, "array"), Some(&vec![Value::Bool(true), Value::Bool(false)]));
    ...
    fn get_bool_element_handles_good_value () {
        ...
        assert_eq!(ParseTools::get_string_element (&array, 2), Some("booga".to_string()));
        assert_eq!(ParseTools::get_array_element (&array, 3), Some(&vec![Value::Bool(true), Value::Bool(false)]));
