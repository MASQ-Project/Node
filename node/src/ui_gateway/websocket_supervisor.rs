...
impl WebSocketSupervisorReal {
        ...
        loop {
            ...
            match message_type {
                ...
                Incoming::Closed(reason) => {
                    info!(
                        &logger,
                        "UI client {} at {} disconnected: {:?}", client_id, peer_addr, reason
                    );
                    let mut locked_inner = inner_arc.lock().expect("WebSocketSupervisor is dead");
    ...
    async fn send_to_clients(
        ...
        let client_id_result_pairs = join_all(clients.iter_mut().map(
            |(client_id, mut client)| async move {
...
impl WebSocketSupervisorFactory for WebsocketSupervisorFactoryReal {
    fn make(
        ...
        // TODO This shouldn't be a Result, since there's no way to fail.
...
mod tests {
    ...
    use masq_lib::messages::{
        ...
        UiDescriptorResponse, UiMessageError, UiShutdownRequest, UiStartOrder, UiUnmarshalError,
        NODE_UI_PROTOCOL,
    };
