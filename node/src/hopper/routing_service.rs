...
impl RoutingService {
    ...
    pub fn route(&self, ibcd: InboundClientData) {
        ...
        let next_hop = match live_package.route.next_hop(self.cryptdes.main().as_ref()) {
            Ok(hop) => hop,
            Err(e) => {
                error!(
                    self.logger,
    ...
    fn route_data_around_again(
        ...
        let (_, next_lcp) = match live_package.into_next_live(self.cryptdes.main().as_ref()) {
            Ok(x) => x,
            Err(e) => {
    ...
    fn to_transmit_data_msg(
        ...
        let (next_hop, next_live_package) =
            match live_package.into_next_live(self.cryptdes.main().as_ref()) {
                Err(e) => {
                    let msg = format!(
        ...
        let next_live_package_enc =
            match encodex(self.cryptdes.main().as_ref(), &next_hop.public_key, &next_live_package) {
                Ok(nlpe) => nlpe,
                Err(e) => {
...
mod tests {
    ...
    use crate::sub_lib::accountant::ReportRoutingServiceProvidedMessage;
    use crate::sub_lib::cryptde::{encodex, CryptDE, NullCryptDE, PlainData, PublicKey};
    ...
    fn dns_resolution_failures_are_reported_to_the_proxy_server() {
        ...
        let route = route_to_proxy_server(&cryptdes.main().public_key(), cryptdes.main().as_ref());
    ...
    fn logs_and_ignores_message_that_cannot_be_decrypted() {
        ...
        let pair = {
            ...
            let pair = Bootstrapper::pub_initialize_cryptdes_for_testing(
        ...
        let route = route_from_proxy_client(pair.main().public_key(), pair.main().as_ref());
        let lcp = LiveCoresPackage::new(
            route,
            encodex(&rogue_cryptde, rogue_cryptde.public_key(), &[42u8]).unwrap(),
        );
        let data_enc = encodex(pair.main().as_ref(), pair.main().public_key(), &lcp).unwrap();
        let inbound_client_data = InboundClientData {
            timestamp: SystemTime::now(),
            client_addr: SocketAddr::from_str("1.2.3.4:5678").unwrap(),
            reception_port: None,
    ...
    fn converts_live_message_to_expired_for_existing_proxy_client() {
        ...
        let payload = make_request_payload(0, main_cryptde().as_ref());
        let lcp = LiveCoresPackage::new(
            route,
            encodex::<MessageType>(
                main_cryptde().as_ref(),
    ...
    fn converts_live_message_to_expired_for_proxy_server() {
        ...
        let route = route_to_proxy_server(&main_cryptde().public_key(), main_cryptde().as_ref());
        ...
        let lcp_a = lcp.clone();
        let lcp_enc = encodex(main_cryptde().as_ref(), main_cryptde().public_key(), &lcp).unwrap();
        let inbound_client_data = InboundClientData {
            timestamp: SystemTime::now(),
    ...
    fn converts_live_gossip_failure_message_to_expired_for_neighborhood() {
        ...
        let lcp = LiveCoresPackage::new(
            route,
            encodex::<MessageType>(main_cryptde().as_ref(), &main_cryptde().public_key(), &payload).unwrap(),
        );
        let data_enc = encodex(main_cryptde().as_ref(), &main_cryptde().public_key(), &lcp).unwrap();
        let inbound_client_data = InboundClientData {
            timestamp: SystemTime::now(),
    ...
    fn passes_on_inbound_client_data_not_meant_for_this_node() {
        ...
        let expected_lcp = lcp_a.into_next_live(main_cryptde().as_ref()).unwrap().1;
    ...
    fn reprocesses_inbound_client_data_meant_for_this_node_and_destined_for_hopper() {
        ...
        let expected_lcp_enc =
            encodex(main_cryptde().as_ref(), &main_cryptde().public_key(), &expected_lcp).unwrap();
        assert_eq!(
            *record,
            InboundClientData {
    ...
    fn route_logs_and_ignores_cores_package_that_demands_proxy_client_routing_with_paying_wallet_that_cant_pay(
        ...
        let payload = ClientRequest(VersionedData::new(
            ...
            &make_request_payload(0, main_cryptde().as_ref()),
        ));
        ...
        let hops = live_hops
            .iter()
            .map(|hop| match hop.encode(&hop.public_key, main_cryptde().as_ref()) {
                Ok(cryptdata) => cryptdata,
                Err(e) => panic!("Couldn't encode hop: {:?}", e),
            })
            .collect();
        let route = Route { hops };
        let icp = IncipientCoresPackage::new(main_cryptde().as_ref(), route, payload, &public_key).unwrap();
        let (lcp, _) = LiveCoresPackage::from_incipient(icp, main_cryptde().as_ref()).unwrap();
        let data_ser = PlainData::new(&serde_cbor::ser::to_vec(&lcp).unwrap()[..]);
        let data_enc = main_cryptde()
            .encode(&main_cryptde().public_key(), &data_ser)
            .unwrap();
    ...
    fn route_logs_and_ignores_cores_package_that_demands_hopper_routing_with_paying_wallet_that_cant_pay(
        ...
        BAN_CACHE.clear();
        init_test_logging();
        let current_key = main_cryptde().public_key().clone();
        let origin_key = PublicKey::new(&[1, 2]);
        ...
        let lcp = LiveCoresPackage::new(
            route,
            encodex(main_cryptde().as_ref(), &destination_key, &payload).unwrap(),
        );
        ...
        F: FnOnce(&CryptDEPair) -> CryptData,
    {
        ...
        let route = Route::single_hop(&PublicKey::new(b"1234"), subject.cryptdes.main().as_ref()).unwrap();
