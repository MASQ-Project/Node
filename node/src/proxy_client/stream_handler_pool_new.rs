...
mod tests {
    ...
    fn non_terminal_payload_can_be_sent_over_existing_connection() {
        ...
        let logger = Logger::new("non_terminal_payload_can_be_sent_over_existing_connection");
        let stream_key = make_meaningless_stream_key();
        ...
        let payload_size = client_request_payload.sequenced_packet.data.len();
        let transmit_chunks_arc = Arc::new(Mutex::new(vec![]));
        let stream_connector = StreamConnectorMock::new()
            .transmit_chunks(&transmit_chunks_arc);
        let system = System::new();
        ...
        subject.resolver = Box::new(ResolverWrapperMock::new());
        subject.stream_connector = stream_connector;
        subject.logger = logger;
        let before = SystemTime::now();

        run_process_package(subject, client_request_payload, Wallet::new("Customer"));
        ...
        let actual_accountant_message = accountant_recording.get_record::<ReportExitServiceProvidedMessage>(0);
        let expected_accountant_message = ReportExitServiceProvidedMessage {
            timestamp: actual_accountant_message.timestamp,
            paying_wallet: Wallet::new("Customer"),
            payload_size,
            service_rate: 100,
            byte_rate: 200
        };
        assert_eq! (actual_accountant_message, expected_accountant_message);
        assert!(actual_accountant_message.timestamp.ge(&before), "{:?} should have been on or after {:?}", actual_accountant_message.timestamp, before);
        assert!(actual_accountant_message.timestamp.le(&after), "{:?} should have been on or before {:?}", actual_accountant_message.timestamp, after);
        assert_eq!(
            transmit_chunks_arc.lock().unwrap().remove(0),
            TransmittedChunk {
                stream_key,
                data: b"These are the times".to_vec()
            }
