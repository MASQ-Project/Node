...
impl AutomapControlFactory for AutomapControlFactoryMock {
    fn make(&self, usual_protocol_opt: Option<AutomapProtocol>, change_handler: ChangeHandler) -> Box<dyn AutomapControl> {
        self.make_params.lock().unwrap().push ((usual_protocol_opt, change_handler));
...
impl AutomapControlFactoryMock {
    ...
    pub fn make_params (mut self, params: &Arc<Mutex<Vec<(Option<AutomapProtocol>, ChangeHandler)>>>) -> Self {
        self.make_params = params.clone();
        self
    }

    pub fn make_result (self, result: AutomapControlMock) -> Self {
