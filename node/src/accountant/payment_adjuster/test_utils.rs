...
// This basically says: visible only within the PaymentAdjuster module
pub(super) mod local_utils {
    ...
    impl CriterionCalculatorMock {
        pub fn calculate_params(
            mut self,
            params: &Arc<Mutex<Vec<QualifiedPayableAccount>>>,
        ) -> Self {
    ...
    impl DisqualificationGaugeMock {
        pub fn determine_limit_params(
            mut self,
            params: &Arc<Mutex<Vec<(u128, u128, u128)>>>,
        ) -> Self {
        ...
        qualified_account
            ...
            .map(|account| {
                let disqualification_limit =
                    DisqualificationArbiter::default().calculate_disqualification_edge(&account);
