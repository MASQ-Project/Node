...
impl CriterionCalculator for BalanceAndAgeCriterionCalculator {
    fn calculate(&self, account: &QualifiedPayableAccount, context: &dyn PaymentAdjusterInner) -> u128 {
        let now = context.now();
        let debt_age_s = now.duration_since(account.payable.last_paid_timestamp).expect("time traveller").as_secs();
        (account.payable.balance_wei - account.payment_threshold_intercept_minor + debt_age_s as u128)
    }
...
mod tests {
    ...
    use crate::accountant::payment_adjuster::criterion_calculators::balance_and_age_calculator::{
        BalanceAndAgeCriterionCalculator,
    };
    use crate::accountant::payment_adjuster::criterion_calculators::CriterionCalculator;
    ...
    fn balance_and_age_criterion_calculator_works() {
        ...
        let qualified_accounts = [0,10,22].into_iter().map(|n| make_non_guaranteed_qualified_payable(n)).collect::<Vec<_>>();
        let payment_adjuster_inner = PaymentAdjusterInnerReal::new(now, None, 123456789);
        let subject = BalanceAndAgeCriterionCalculator::default();

        let computed_criteria = qualified_accounts.iter().map(|qualified_account|subject.calculate(qualified_account, &payment_adjuster_inner)).collect::<Vec<_>>();

        let zipped = qualified_accounts.into_iter().zip(computed_criteria.into_iter());
        zipped.into_iter().for_each(|(account, actual_criterion)|{
            let debt_age_s = now.duration_since(account.payable.last_paid_timestamp).unwrap().as_secs();
            let expected_criterion = account.payable.balance_wei - account.payment_threshold_intercept_minor + debt_age_s as u128;
