...
use crate::accountant::payment_adjuster::auxiliary_fns::{
    compute_fractions_preventing_mul_coeff, criteria_total, cut_back_by_gas_count_limit,
    exhaust_cw_balance_as_much_as_possible,
    find_disqualified_account_with_smallest_proposed_balance, log_2, rebuild_accounts,
    sort_in_descendant_order_by_weights, sum_as, x_or_1,
};
...
use crate::accountant::payment_adjuster::diagnostics::{
    diagnostics, diagnostics_collective, CriteriaWithDiagnostics, DiagnosticsSetting,
    AGE_SINGLETON, BALANCE_SINGLETON, EXPONENTS_OF_10_AS_VALUES_FOR_X_AXIS,
};
...
impl PaymentAdjusterReal {
    ...
    fn run_full_adjustment_procedure(
        ...
        let sorted_accounts_with_individual_criteria =
            self.pair_accounts_with_summed_criteria(unresolved_qualified_accounts);
    ...
    fn pair_accounts_with_summed_criteria(
        &self,
    ...
    fn run_adjustment_by_criteria_recursively(
        ...
        log_info_for_disqualified_account(
            &self.logger,
        ...
        let result: Vec<PayableAccount> = previously_resolved_qualified_accounts
            .into_iter()
    ...
    fn apply_criteria(
        ...
        let weights_and_accounts = accounts_with_zero_criteria
            .map_criteria(AgeCriterionCalculator::new(self))
            .map_criteria(BalanceCriterionCalculator::new(self));
            ...
            Some(limitation_by_gas) => {
                ...
                match Self::check_need_of_masq_balances_adjustment(
                    ...
                    true => {
                        ...
                        AdjustmentCompletion::Continue(result_awaiting_verification)
                    }
                    false => {
                        let finalized_accounts = rebuild_accounts(weighted_accounts_cut_by_gas);
                        AdjustmentCompletion::Finished(finalized_accounts)
                    }
                }
            }
            None => {
    ...
    fn handle_masq_token_adjustment(
        ...
        match self.recreate_accounts_with_proportioned_balances(
            ...
            AccountsRecreationResult::InsignificantAccounts {
                ...
                disqualified_account_opt: Some(disqualified),
            },
    ...
    fn recreate_accounts_with_proportioned_balances(
        ...
        let verified_and_exhaustive =
            exhaust_cw_balance_as_much_as_possible(verified_accounts, unallocated_cw_masq_balance);

        AccountsRecreationResult::AllAccountsCleanlyProcessed(verified_and_exhaustive)
    }
    ...
    fn consider_account_disqualification_from_percentage_insignificance(
        ...
        logger: &Logger,
    ) -> Either<Vec<AdjustedAccountBeforeFinalization>, AccountsRecreationResult> {
            ...
            )
        {
            ...
            let (single_disqualified, remaining) = unfinalized_adjusted_accounts.into_iter().fold(
                init,
                |(disqualified_acc_opt, mut remaining_accounts), current_account| {
    ...
    fn maybe_find_an_account_to_disqualify_in_this_iteration(
        ...
        let disqualification_suspected_accounts =
            Self::list_accounts_under_the_disqualification_limit(unfinalized_adjusted_accounts);
    ...
    fn list_accounts_under_the_disqualification_limit(
        unfinalized_adjusted_accounts: &[AdjustedAccountBeforeFinalization],
    ) -> Vec<&AdjustedAccountBeforeFinalization> {
        unfinalized_adjusted_accounts
            .iter()
    ...
    fn handle_possibly_outweighed_account(
        ...
        let (outweighed_with_already_made_criteria, passing_through) =
            unfinalized_adjusted_accounts.into_iter().fold(
                init,
    ...
    fn adjust_balance_of_outweighed_accounts_to_cw_balance_if_necessary(
        ...
        {
            todo!("will we ever be here again?");
...
mod tests {
    use crate::accountant::database_access_objects::payable_dao::PayableAccount;
    use crate::accountant::payment_adjuster::auxiliary_fns::{
        compute_fractions_preventing_mul_coeff, criteria_total, log_2,
    };
    ...
    use crate::accountant::payment_adjuster::{
        AccountsRecreationResult, AdjustedAccountBeforeFinalization, Adjustment, AnalysisError,
        ...
        BALANCE_LOG_2_ARG_DIVISOR, PRINT_PARTIAL_COMPUTATIONS_FOR_DIAGNOSTICS,
    };
    use crate::accountant::scanners::payable_scan_setup_msgs::{
        FinancialAndTechDetails, PayablePaymentSetup, StageData,
    };
    ...
    use thousands::Separable;
    use web3::types::U256;
    use crate::accountant::payment_adjuster::criteria_calculators::AgeCriterionCalculator;
    ...
    fn list_accounts_under_the_disqualification_limit_employs_manifest_consts_of_insignificance() {
        ...
        let result = PaymentAdjusterReal::list_accounts_under_the_disqualification_limit(
    ...
    fn apply_criteria_returns_accounts_sorted_by_final_weights_in_descending_order() {
        ...
        let weights_and_accounts = subject.pair_accounts_with_summed_criteria(qualified_payables);
    ...
    fn only_the_least_demanding_disqualified_account_is_picked_at_a_time_even_though_more_of_them_can_be_found(
        ...
        let accounts_with_individual_criteria =
            subject.pair_accounts_with_summed_criteria(vec![account_1, account_2, account_3]);
        let criteria_total = criteria_total(&accounts_with_individual_criteria);
    ...
    fn small_debt_with_extreme_age_is_paid_outweighed_but_not_with_more_money_than_required() {
        ...
        let criteria_and_accounts = subject.pair_accounts_with_summed_criteria(qualified_payables);
    ...
    fn an_accounts_never_becomes_outweighed_while_cw_balance_does_not_have_enough_balance_for_it_because_disqualified_accounts_come_handled_first(
        ...
        let accounts_with_individual_criteria =
            subject.pair_accounts_with_summed_criteria(accounts);
        let criteria_total = criteria_total(&accounts_with_individual_criteria);
...
|0x0000000000000000000000000000000000616263 111000000000000"
    ...
    fn adjust_payments_when_not_enough_masq_to_pay_at_least_half_of_each_account() {
        ...
        // first we disqualify the smallest, but also last at that balance, account which is account 2,
        // therefore only account 1 remains and soon after qualifies
...
|0x0000000000000000000000000000000000646566 55000000000"
    ...
    fn emulation_of_the_actual_adjustment_algorithm(
        ...
        .flatten()
        .collect::<Vec<_>>();
        //TODO continue here ... fix it so that, if possible, you don't have to use the whole subject here, which has its over-time varying inner state
        let age_criteria = AgeCriterionCalculator::;
