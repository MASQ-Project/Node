...
use crate::accountant::scanners::mid_scan_msg_handling::payable_scanner::msgs::{QualifiedPayablesRawPack, QualifiedPayablesRipePack, QualifiedPayablesWithGasPrice};
...
impl BlockchainAgentWeb3 {
    pub fn new(
        ...
        let current_gas_price_with_margin = increase_gas_price_by_margin(latest_gas_price_wei, chain);
        
        let (ripe_qualified_payables, gas_price_aggregated_wei) = raw_qualified_payables.payables.into_iter().fold(
            (
                QualifiedPayablesRipePack{payables: vec![]},
                0,
            ),
            |(mut ripe_qualified_payables, mut gas_price_aggregated_wei), raw_q_payable| {
                let picked_gas_price_wei = match raw_q_payable.previous_attempt_gas_price_minor_opt {
                    None => current_gas_price_with_margin,
                    Some(previous_price) if current_gas_price_with_margin < previous_price => previous_price,
                    Some(_) => current_gas_price_with_margin 
                };
                let ripe_qualified_payable = QualifiedPayablesWithGasPrice::new(raw_q_payable.payable, picked_gas_price_wei);
                ripe_qualified_payables.payables.push(ripe_qualified_payable);
                gas_price_aggregated_wei += picked_gas_price_wei;
                (ripe_qualified_payables, gas_price_aggregated_wei)
            });

        let estimated_gas_price_wei = gas_price_aggregated_wei * (gas_limit_const_part + WEB3_MAXIMAL_GAS_LIMIT_MARGIN);
...
mod tests {
    ...
    use web3::types::{U256};
    use crate::accountant::scanners::mid_scan_msg_handling::payable_scanner::msgs::{QualifiedPayablesWithGasPrice, QualifiedPayablesBeforeGasPricePick, QualifiedPayablesRawPack, QualifiedPayablesRipePack};
