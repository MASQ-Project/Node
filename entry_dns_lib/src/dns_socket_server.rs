...
impl Future for DnsSocketServer {
    ...
    fn poll(&mut self) -> Result<Async<<Self as Future>::Item>, <Self as Future>::Error> {
        ...
        loop {
            let mut buffer = self.buf.expect("Missing buffer - was initialize_as_privileged called?");
            let (len, socket_addr) = match self.socket_wrapper.recv_from(buffer.borrow_mut()) {
                Ok(Async::Ready((len, socket_addr))) => {
                    (len, socket_addr)
                },
                Ok(Async::NotReady) => {
                ...
                Err(e) => {
                    logger.error(format!("Unrecoverable error receiving from UdpSocket: {}", e));
                    return Err(());
                },
            };
            let processor_unwrapped = self.processor.as_ref().expect("Missing Processor - was initialized_as_privileged called?");
            let response_length = processor_unwrapped.process(buffer.borrow_mut(), len, &socket_addr, &logger);
            match self.socket_wrapper.send_to(&buffer[0..response_length], socket_addr) {
                Err(e) => {
...
impl SocketServer for DnsSocketServer {
    ...
    fn initialize_as_unprivileged(&mut self) {
        let processor_real = ProcessorReal::new(self.dns_target.expect("Missing dns_target - was initialize_as_privileged called?"));
...
mod tests {
    ...
    impl UdpSocketWrapperTrait for UdpSocketWrapperMock {
        ...
        fn recv_from(&mut self, buf: &mut [u8]) -> Result<Async<(usize, SocketAddr)>, Error> {
            ...
            let result = self.recv_from_results.lock().unwrap().borrow_mut().remove(0);
    ...
    impl UdpSocketWrapperMock {
        fn new (buf: &[u8]) -> UdpSocketWrapperMock {
            ...
            result.guts.lock().unwrap().borrow_mut ().deref_mut ().buf.copy_from_slice (&buf);
            result
        }
    ...
    fn complains_about_missing_dns_target () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_target"), String::from ("--something_else")),
                                   &mut holder.streams ());
    }
    ...
    fn complains_about_missing_dns_target_at_end () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("irrelevant"), String::from ("--dns_target")),
                                   &mut holder.streams ());
    }
    ...
    fn complains_about_dns_target_with_too_many_dots () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_target"), String::from ("lots.and.lots.of.dots")),
                                   &mut holder.streams ());
    }
    ...
    fn complains_about_dns_target_with_too_few_dots () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_target"), String::from ("only.two.dots")),
                                   &mut holder.streams ());
    }
    ...
    fn complains_about_dns_target_with_nonnumeric_components () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_target"), String::from ("123.124.125.booga")),
                                   &mut holder.streams ());
    }
    ...
    fn complains_about_dns_target_with_numeric_components_too_large () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_target"), String::from ("123.124.125.256")),
                                   &mut holder.streams ());
    }
    ...
    fn accepts_valid_dns_target () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_target"), String::from ("123.124.125.126")),
                                   &mut holder.streams ());
    ...
    fn complains_about_missing_dns_port () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_port"), String::from ("--something_else")),
                                   &mut holder.streams ());
    }
    ...
    fn complains_if_dns_server_port_is_not_numeric () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_port"), String::from ("booga")),
                                   &mut holder.streams ());
    }
    ...
    fn complains_if_dns_server_port_is_too_small () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_port"), String::from ("0")),
                                   &mut holder.streams ());
    ...
    fn complains_if_dns_server_port_is_too_large () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_port"), String::from ("65536")),
                                   &mut holder.streams ());
    ...
    fn accepts_valid_dns_port () {
        ...
        subject.initialize_as_privileged(&vec!(String::from ("--dns_port"), String::from ("5454")),
                                   &mut holder.streams ());
    ...
    fn serves_multiple_requests_then_short_circuit_on_error() {
        ...
        let (log, mut buf) = {
            let socket_wrapper = make_socket_wrapper_mock();
            socket_wrapper.recv_from_results.lock().unwrap().push(Ok(Async::Ready((socket_wrapper.guts.lock().unwrap().borrow().buf.len(), SocketAddr::from_str("0.0.0.0:0").unwrap()))));
            socket_wrapper.recv_from_results.lock().unwrap().push(Ok(Async::Ready((socket_wrapper.guts.lock().unwrap().borrow().buf.len(), SocketAddr::from_str("1.0.0.0:0").unwrap()))));
            socket_wrapper.recv_from_results.lock().unwrap().push(Ok(Async::Ready((socket_wrapper.guts.lock().unwrap().borrow().buf.len(), SocketAddr::from_str("2.0.0.0:0").unwrap()))));
            socket_wrapper.recv_from_results.lock().unwrap().push(Err(Error::from(ErrorKind::BrokenPipe)));

            socket_wrapper.send_to_results.lock().unwrap().push(Ok(Async::Ready(12)));
            socket_wrapper.send_to_results.lock().unwrap().push(Ok(Async::Ready(12)));
            socket_wrapper.send_to_results.lock().unwrap().push(Ok(Async::Ready(12)));
    ...
    fn poll_handles_error_sending_to_udp_socket_wrapper() {
        ...
        let socket_wrapper = make_socket_wrapper_mock();
        socket_wrapper.recv_from_results.lock().unwrap().push(Ok(Async::Ready((socket_wrapper.guts.lock().unwrap().borrow().buf.len(), SocketAddr::from_str("0.0.0.0:0").unwrap()))));
        socket_wrapper.send_to_results.lock().unwrap().push(Err(Error::from (ErrorKind::BrokenPipe)));
    ...
    fn make_instrumented_subject (socket_wrapper: Box<UdpSocketWrapperMock>) -> DnsSocketServer {
        DnsSocketServer {dns_target: None, socket_wrapper, processor: None, buf: None,}
    }
}