...
impl Query {
    ...
    pub fn new_for_test (name: String, qtype: u16, qclass: u16, length: usize) -> Query {
        Query {name, qtype, qclass, length}
    }
    ...
    fn find_end (buf: &[u8], offset: usize, length: usize) -> Option<usize> {
        ...
        return if query_end > length {None} else {Some (query_end)}
    }

    pub fn get_query_name (&self) -> &str {&self.name}

    pub fn get_query_type (&self) -> u16 {self.qtype}

    pub fn get_query_class (&self) -> u16 {self.qclass}

    fn get_length (&self) -> usize {self.length}
}
...
impl ResourceRecord {
    ...
    fn new (buf: &[u8], offset: usize, buflen: usize) -> Option<ResourceRecord> {
        ...
        if offset + length > buflen {return None}
        let rdata = Vec::from (&buf[(rdata_begin)..(rdata_end)]);
        Some (ResourceRecord {name, rtype, rclass, time_to_live, rdata, length})
    }

    pub fn new_for_test (name: String, rtype: u16, rclass: u16, time_to_live: u32, rdata: Vec<u8>, length: usize) -> ResourceRecord {
        ResourceRecord {name, rtype, rclass, time_to_live, rdata, length}
    }
    ...
    fn find_end (buf: &[u8], offset: usize, length: usize) -> Option<usize> {
        ...
        if record_end > length {None} else {Some (record_end)}
    }

    pub fn get_name(&self) -> &str { &self.name }

    pub fn get_resource_type(&self) -> u16 { self.rtype }

    pub fn get_resource_class(&self) -> u16 { self.rclass }

    pub fn get_time_to_live(&self) -> u32 { self.time_to_live }

    pub fn get_rdata(&self) -> &[u8] { &self.rdata }
...
impl<'a> PacketFacade<'a> {
    pub fn new (buf: &mut [u8], length: usize) -> PacketFacade {
        PacketFacade {buf, length: length }
    }
    ...
    pub fn set_query (&mut self, v: bool) -> bool {
        if self.buf.len () < 3 {return false}
    ...
    pub fn set_recursion_available (&mut self, v: bool) -> bool {
        if self.buf.len () < 4 {return false}
    ...
    pub fn add_query (&mut self, name: &str, query_type: u16, query_class: u16) -> bool {
        ...
        if !PacketFacade::u16_to (query_type, self.buf, offset) { return false };
        if !PacketFacade::u16_to (query_class, self.buf, offset + 2) { return false };
    ...
    pub fn get_answers(&self) -> Option<Vec<ResourceRecord>> {
        ...
        for _i in 0..count {
            let record: ResourceRecord = try_opt! (ResourceRecord::new (self.buf, offset, self.length));
    ...
    pub fn add_answer(&mut self, name: &str, resource_type: u16,
                      resource_class: u16, time_to_live: u32, rdata: &[u8]) -> bool {
        let begin = try_flg! (self.find_answers_end());
        try_flg! (self.write_resource_record (begin, name, resource_type, resource_class, time_to_live, rdata));
    ...
    pub fn add_authority(&mut self, name: &str, resource_type: u16,
                         resource_class: u16, time_to_live: u32, rdata: &[u8]) -> bool {
    ...
    pub fn add_additional (&mut self, name: &str, resource_type: u16,
                           resource_class: u16, time_to_live: u32, rdata: &[u8]) -> bool {
    ...
    fn req_len_opt<T, F> (&self, length: usize, closure: F) -> Option<T>
        ...
        if (self.length < length) || (self.buf.len () < length) {None} else {Some (closure ())}
    }

    fn req_len_flg<F> (&mut self, length: usize, mut closure: F) -> bool
        where F: FnMut (&mut PacketFacade) -> () {
        if (self.length < length) || (self.buf.len () < length) {false} else {closure (self); true}
    }

    fn u16_from (buf: &[u8], start: usize, buflen: usize) -> Option<u16> {
        if buflen < start + 2 {return None}
        Some (((buf[start] as u16) << 8) + (buf[start + 1] as u16))
    }

    fn u16_to (value: u16, buf: &mut [u8], start: usize) -> bool {
        if start + 2 > buf.len () {return false;}
    ...
    fn write_resource_record (&mut self, start: usize, name: &str, resource_type: u16,
                              resource_class: u16, time_to_live: u32, rdata: &[u8]) -> Option<usize> {
        let string_end = try_opt! (PacketFacade::add_string_seq (self.buf, start, name));
        if !PacketFacade::u16_to (resource_type, self.buf, string_end) {return None}
        if !PacketFacade::u16_to (resource_class, self.buf, string_end + 2) {return None}
        if !PacketFacade::u32_to (time_to_live, self.buf, string_end + 4) {return None}
        if !PacketFacade::u16_to (rdata.len () as u16, self.buf, string_end + 8) {return None}
        let rdata_begin = string_end + 10;
        let rdata_end = rdata_begin + rdata.len();
        if rdata_end > self.buf.len () {return None};
    ...
    fn find_string_seq_end (buf: &[u8], offset: usize, buflen: usize) -> Option<usize> {
        ...
        loop {
            if local_offset > buflen {return None}
            let length = buf[local_offset] as usize;
            if length == 0x00 {return Some (local_offset + 1)}
    ...
    fn extract_string_seq (buf: &[u8], offset: usize, buflen: usize) -> Option<(String, usize)> {
        ...
        loop {
            if local_offset >= buflen {return None;}
            let length = buf[local_offset] as usize;
            if length == 0x00 {return Some ((result, local_offset + 1))}
    ...
    fn add_string_seq (buf: &mut [u8], offset: usize, string: &str) -> Option<usize> {
        ...
        if string.len () > 0 {
            for part in string.split('.') {
                ...
                if (local_offset + bytes.len () + 1) > buf.len () {return None}
        ...
        if local_offset >= buf.len () {return None};
...
mod tests {
    ...
    fn can_access_answers () {
        ...
        assert_eq! (first_record.get_rdata (), u8vec (&vec![0xCD, 0xEF, 0x01, 0x23]));
        ...
        assert_eq! (second_record.get_rdata (), u8vec (&vec![0x56, 0x78, 0x9A, 0xBC]));
    ...
    fn can_access_authorities () {
        ...
        let length = {
            ...
            adder.add_bytes (&vec![0x00, 0x12, 0x34, 0x43, 0x21]); // null query
            adder.add_bytes (&vec![0x00, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0, 0x12, 0x34, 0x00, 0x00]); // null answer
    ...
    fn can_access_additionals () {
        ...
        let length = {
            ...
            adder.add_bytes (&vec![0x00, 0x12, 0x34, 0x43, 0x21]); // null query
            adder.add_bytes (&vec![0x00, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0, 0x12, 0x34, 0x00, 0x00]); // null answer
            adder.add_bytes (&vec![0x00, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x00]); // null authority
    ...
    fn can_add_answers () {
        ...
        let get_length = {
            ...
            assert_eq! (subject.add_answer("www.domain.com", 0x1234, 0x2345, 0x12345678, &vec![0x00, 0x05, 0x10, 0x15]), true);
            assert_eq! (subject.add_answer("xyz.fiddles.org", 0x3456, 0x4567, 0x87654321, &vec![0x20, 0x25, 0x30, 0x35]), true);
    ...
    fn can_add_authorities () {
        ...
        let get_length = {
            ...
            assert_eq! (subject.add_query("", 0xFEDC, 0xBA98), true);
            assert_eq! (subject.add_answer("", 0xFEDC, 0xBA98, 0x76543210, &vec![]), true);
            assert_eq! (subject.add_authority("www.domain.com", 0x1234, 0x2345, 0x12345678, &vec![0x00, 0x05, 0x10, 0x15]), true);
            assert_eq! (subject.add_authority("xyz.fiddles.org", 0x3456, 0x4567, 0x87654321, &vec![0x20, 0x25, 0x30, 0x35]), true);
        ...
        checker.check_bytes (&vec![0x00, 0xFE, 0xDC, 0xBA, 0x98]); // query for spacing
        checker.check_bytes (&vec![0x00, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x00]); // answer for spacing
    ...
    fn can_add_additionals () {
        ...
        let get_length = {
            ...
            assert_eq! (subject.add_query("", 0xFEDC, 0xBA98), true);
            assert_eq! (subject.add_answer("", 0xFEDC, 0xBA98, 0x76543210, &vec![]), true);
            assert_eq! (subject.add_authority("", 0xFEDC, 0xBA98, 0x76543210, &vec![]), true);
            assert_eq! (subject.add_additional("www.domain.com", 0x1234, 0x2345, 0x12345678, &vec![0x00, 0x05, 0x10, 0x15]), true);
            assert_eq! (subject.add_additional("xyz.fiddles.org", 0x3456, 0x4567, 0x87654321, &vec![0x20, 0x25, 0x30, 0x35]), true);
        ...
        checker.check_bytes (&vec![0x00, 0xFE, 0xDC, 0xBA, 0x98]); // query for spacing
        checker.check_bytes (&vec![0x00, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x00]); // answer for spacing
        checker.check_bytes (&vec![0x00, 0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x00]); // authority for spacing
    ...
    fn writing_resource_record_adjusts_subject_length () {
        ...
        let result = subject.write_resource_record (12,"booga",
                                                    0x1234, 0x5678, 0x9ABCDEF0, &vec![0x24, 0x36]);
    ...
    fn writing_resource_record_returns_none_if_name_end_indicator_busts_length () {
        ...
        let result = subject.write_resource_record (12, "x",
                                                    0x1234, 0x5678, 0x9ABCDEF0, &vec![0x48]);
    ...
    fn writing_resource_record_returns_none_if_rdata_busts_length () {
        ...
        let result = subject.write_resource_record (12, "",
                                                    0x1234, 0x5678, 0x9ABCDEF0, &vec![0x48]);
