...
fn position_in_range (digit: u8, first: u8, last: u8) -> Option<u8> {
    if digit < first {return None}
    if digit > last {return None}
    Some (digit - first)
}
...
mod tests {
    ...
    pub fn returns_data_for_single_capital_hexadecimal_digit () {
        ...
        assert_eq! (result, Some (ChunkOffsetLength {offset: 4, length: 15 + 3}));
    }
    ...
    pub fn returns_data_for_eight_capital_hexadecimal_digits () {
        ...
        assert_eq! (result, Some (ChunkOffsetLength {offset: 23, length: 0x89ABCDEF + 10}));
    }
