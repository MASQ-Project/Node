...
impl HttpPacketFramer {
    ...
    fn take_packet_frame (&mut self) -> Option<FramedChunk> {
        if self.framer_state.packet_progress_state == PacketProgressState::SeekingPacketStart {
            if !self.start_finder.seek_packet_start (&mut self.framer_state) {return None}
        }
        if self.framer_state.packet_progress_state == PacketProgressState::SeekingBodyStart {
            if !self.seek_body_start() { return None }
        }
        ...
        while self.framer_state.packet_progress_state == PacketProgressState::SeekingBodyStart {
            match index_of(&self.framer_state.data_so_far[..], "\r\n".as_bytes()) {
                Some(line_end) => {
                    ...
                    if self.framer_state.content_length == 0 {self.check_for_content_length(&line)}
                    if self.framer_state.transfer_encoding_chunked == ChunkExistenceState::Standard {self.check_for_transfer_encoding(&line)}
    ...
    fn check_for_transfer_encoding (&mut self, line: &Vec<u8>) {
        if !line.starts_with ("Transfer-Encoding:".as_bytes ()) {return}
        let string = match String::from_utf8 (line.clone ()) {
            Err (_) => {self.discard_current_request (); return},
            Ok (string) => string
        };
    ...
    fn take_frame_while_seeking_length_header(&mut self) -> Option<FramedChunk> {
        match framer_utils::find_chunk_offset_length(&self.framer_state.data_so_far[..]) {
            ...
            Some(chunk_offset_length) => {
                self.framer_state.data_so_far = self.framer_state.data_so_far.split_off(chunk_offset_length.offset);
                if (chunk_offset_length.length == 3) && (self.framer_state.data_so_far[chunk_offset_length.offset] == ('0' as u8)) {
                    self.framer_state.chunk_progress_state = ChunkProgressState::SeekingEndOfFinalChunk;
                    self.framer_state.chunk_size = None;
                }
                else {
    ...
    fn take_frame_while_seeking_end_of_chunk(&mut self) -> Option<FramedChunk> {
        ...
        let temp = self.framer_state.data_so_far.split_off (chunk_size + CRLF.len ());
        ...
        if &result_data[(result_data.len () - CRLF.len ())..] != CRLF {
            ...
            self.framer_state.data_so_far.insert (0, result_data[result_data.len () - 1]);
            self.framer_state.data_so_far.insert (0, result_data[result_data.len () - 2]);
    ...
    fn take_frame_while_seeking_end_of_final_chunk (&mut self) -> Option<FramedChunk> {
        match index_of (&self.framer_state.data_so_far[..], DOUBLE_CRLF) {
            Some (offset) => {
                let temp = self.framer_state.data_so_far.split_off (offset + DOUBLE_CRLF.len ());
...
GOOD_FIRST_LINE\r\nContent-Length: 5\r\n\r\ngooba".as_bytes()));
        ...
        assert_eq! (result, None);
        assert_eq! (to_string (&subject.framer_state.data_so_far), "\r\nboogaGOOD_FIRST_LINE\r\nContent-Length: 5\r\n\r\ngooba");
    }
...
GOOD_FIRST_LINE\r\nContent-Length: 5\r\n\r\ngooba".as_bytes();
    ...
    fn transfer_encoding_is_standard_if_not_mentioned () {
        ...
        assert_eq! (subject.framer_state.transfer_encoding_chunked, ChunkExistenceState::Standard);
    }
    ...
    fn transfer_encoding_is_chunked_if_header_is_present_and_mentions_chunked_alone () {
        ...
        assert_eq! (result, None);
        assert_eq! (subject.framer_state.transfer_encoding_chunked, ChunkExistenceState::ChunkedResponse);
    }
    ...
    fn transfer_encoding_is_chunked_if_header_is_present_and_mentions_chunked_among_others () {
        ...
        assert_eq! (subject.framer_state.transfer_encoding_chunked, ChunkExistenceState::Chunk);
    }
    ...
    fn ignores_garbage_except_for_last_nine_chars () {
        ...
        assert_eq! (result, None);
        assert_eq! (subject.framer_state.data_so_far, Vec::from (&b"n's souls"[..]));
        assert_eq! (subject.framer_state.chunk_progress_state, ChunkProgressState::SeekingLengthHeader);
    ...
    fn ignores_hexadecimal_data_except_for_last_nine_chars () {
        ...
        assert_eq! (result, None);
        assert_eq! (subject.framer_state.data_so_far, Vec::from (&b"98765432\r"[..]));
        assert_eq! (subject.framer_state.chunk_progress_state, ChunkProgressState::SeekingLengthHeader);
        assert_eq! (subject.framer_state.chunk_size, None);
    }
    ...
    fn senses_beginning_properly () {
        ...
        assert_eq! (result, None);
        assert_eq! (subject.framer_state.data_so_far, Vec::from (&b"98765432\r\nbeginning of content"[..]));
        assert_eq! (subject.framer_state.chunk_progress_state, ChunkProgressState::SeekingEndOfChunk);
    ...
    fn frames_single_chunk () {
        ...
        assert_eq! (result, Some (FramedChunk {chunk: Vec::from (&b"13\r\nnineteen characters\r\n"[..]), last_chunk: false}));
        assert_eq! (subject.framer_state.data_so_far, Vec::from (&b"11\r\nanother"[..]));
        assert_eq! (subject.framer_state.chunk_progress_state, ChunkProgressState::SeekingLengthHeader);
        assert_eq! (subject.framer_state.chunk_size, None);
    }
    ...
    fn frames_multiple_chunks_even_unterminated_ones () {
        ...
        assert_eq! (result1, Some (FramedChunk {chunk: Vec::from (&b"13\r\nnineteen characters\r\n"[..]), last_chunk: false}));
        // unterminated; will cause error in browser, but that's appropriate
        assert_eq! (result2, Some (FramedChunk {chunk: Vec::from (&b"16\r\nanother few characters"[..]), last_chunk: false}));
        assert_eq! (result3, None);
        assert_eq! (subject.framer_state.data_so_far, Vec::from (&b"12\r\nand one"[..]));
        assert_eq! (subject.framer_state.chunk_progress_state, ChunkProgressState::SeekingEndOfChunk);
