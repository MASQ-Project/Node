...
const HTTP_RESPONSE_TEMPLATE: &str =
...
mod tests {
    ...
    fn creates_appropriate_error_page () {
        ...
        assert_eq! (result.contains ("<h2>Title: I'm a title</h2>"), true, "{}", result);
        assert_eq! (result.contains ("<h3>Subtitle: I'm a subtitle</h3>"), true, "{}", result);
        assert_eq! (result.contains ("<p>and I'm content</p>"), true, "{}", result);
    }
    ...
    fn creates_appropriate_error_response () {
        ...
        assert_eq! (result_string.contains ("HTTP/1.1 503 Routing Error"), true, "{}", result_string);
        assert_eq! (result_string.contains (format! ("Content-Length: {}", error_page.len ()).as_str ()), true, "{}", result_string);
        assert_eq! (result_string.contains ("<p>and I'm content</p>"), true, "{}", result_string);
    }
}
