...
fn from (binary: &[u8], type_name: &str) -> DataHunk {
    if binary.len () < 16 {
        panic!("A {} must be at least 16 bytes long to parse into a DataHunk, not {}", type_name, binary.len())
    }
    let from = SocketAddr::new (IpAddr::V4 (Ipv4Addr::new (
        binary[0], binary[1], binary[2], binary[3]
    )), ((binary[4] as u16) << 8) + (binary[5] as u16));
    let to = SocketAddr::new (IpAddr::V4 (Ipv4Addr::new (
...
mod tests {
    ...
    #[test]
    #[should_panic (expected = "A Vec<u8> must be at least 16 bytes long to parse into a DataHunk, not 15")]
    ...
    #[test]
    #[should_panic (expected = "Binary data suggested that a DataHunk payload should be 0 bytes long, but 1 bytes were provided")]
    ...
    fn data_hunks_become_vecs () {
        ...
        assert_eq! (result, vec! (1, 2, 3, 4, 5, 6, 2, 3, 4, 5, 6, 7, 0, 0, 0, 4, 4, 3, 2, 1));
    }
}
