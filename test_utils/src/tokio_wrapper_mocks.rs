...
impl AsyncRead for ReadHalfWrapperMock {
    fn poll_read(&mut self, buf: &mut [u8]) -> Result<Async<usize>, io::Error> {
        if self.poll_read_results.is_empty () {panic! ("ReadHalfWrapperMock: poll_read_results is empty")}
...
impl ReadHalfWrapperMock {
    ...
    pub fn poll_read_result (mut self, data: Vec<u8>, result: Result<Async<usize>, io::Error>) -> ReadHalfWrapperMock {
...
impl AsyncWrite for WriteHalfWrapperMock {
    fn poll_write(&mut self, buf: &[u8]) -> Result<Async<usize>, io::Error> {
        ...
        if self.poll_write_results.is_empty () {panic! ("WriteHalfWrapperMock: poll_write_results is empty")}
...
impl WriteHalfWrapperMock {
    ...
    pub fn poll_write_params (mut self, params_arc: &Arc<Mutex<Vec<Vec<u8>>>>) -> WriteHalfWrapperMock {
        self.poll_write_params = params_arc.clone ();
        self
    }

    pub fn poll_write_result (mut self, result: Result<Async<usize>, io::Error>) -> WriteHalfWrapperMock {
